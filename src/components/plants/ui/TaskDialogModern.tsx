...
export const TaskDialogModern: React.FC<TaskDialogModernProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[var(--bg-secondary)] rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden border-2 border-[#2AD368]/20 shadow-2xl flex flex-col">
        {/* Header */}
        <div className="border-b border-[var(--border-color)] p-4 md:p-6 bg-[var(--bg-primary)]">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={handleClose}
              className="size-10 rounded-full bg-[var(--bg-secondary)] border border-[var(--border-color)] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-all"
            >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 space-y-4">
          {/* Type de tâche */}
          <div>
            <label className="text-xs text-[var(--text-secondary)] font-semibold mb-2 block">Type de tâche *</label>
            <select
              value={taskType}
              onChange={(e) => setTaskType(e.target.value)}
              className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-3 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[#2AD368] transition-all"
            >
          ...
          {/* Titre */}
          <div>
            <label className="text-xs text-[var(--text-secondary)] font-semibold mb-2 block">Titre *</label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="Ex: Arroser les tomates"
              className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-3 py-3 text-[var(--text-primary)] placeholder-[var(--text-secondary)] focus:outline-none focus:border-[#2AD368] transition-all"
            />
          </div>

          {/* Description */}
          <div>
            <label className="text-xs text-[var(--text-secondary)] font-semibold mb-2 block">Description</label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              rows={3}
              placeholder="Ajoutez des détails sur cette tâche..."
              className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-3 py-3 text-[var(--text-primary)] placeholder-[var(--text-secondary)] focus:outline-none focus:border-[#2AD368] transition-all resize-none"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Date d'échéance */}
            <div>
              <label className="text-xs text-[var(--text-secondary)] font-semibold mb-2 block">Date d'échéance *</label>
              <input
                type="date"
                value={dueDate}
            ...
            {/* Fréquence */}
            <div>
              <label className="text-xs text-[var(--text-secondary)] font-semibold mb-2 block">Fréquence (jours)</label>
              <input
                type="number"
                value={frequencyDays}
                ...
                className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-3 py-3 text-[var(--text-primary)] placeholder-[var(--text-secondary)] focus:outline-none focus:border-[#2AD368] transition-all"
              />
              <p className="text-xs text-[var(--text-secondary)] mt-1">Laissez vide pour une tâche unique</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Priorité */}
            <div>
              <label className="text-xs text-[var(--text-secondary)] font-semibold mb-2 block">Priorité</label>
              <select
                value={priority}
        ...
        {/* Footer */}
        <div className="border-t border-[var(--border-color)] p-4 md:p-6 flex flex-wrap gap-3 justify-end bg-[var(--bg-primary)]">
          <button
            onClick={handleClose}
            disabled={loading}
            className="px-6 py-2.5 rounded-xl border border-[var(--border-color)] text-[var(--text-secondary)] font-semibold hover:text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] transition-all disabled:opacity-50"
          >
            Annuler
          </button>
          <button
